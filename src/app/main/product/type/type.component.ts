...
export class TypeComponent extends BaseComponent implements OnInit {
  ...
  onSubmit(value) {
    ...
    if (this.isCreate) {
      this.getEncodeFromImage(this.file_image).subscribe((data: any): void => {
        ...
        this._api.post('/api/loaisanpham/create-lsp', tmp).takeUntil(this.unsubscribe).subscribe(res => {
      ...
      this.getEncodeFromImage(this.file_image).subscribe((data: any): void => {
        ...
        let tmp = {
          Tenloai: value.Tenloai,
          Maloai: this.item.Maloai,
        };
        this._api.post('/api/loaisanpham/update-lsp', tmp).takeUntil(this.unsubscribe).subscribe(res => {
  ...
  onDelete(row) {
    this._api.post('/api/loaisanpham/delete-lsp',{Maloai:row.Maloai}).takeUntil(this.unsubscribe).subscribe(res => {
  ...
  public openUpdateModal(row) {
    ...
    setTimeout(() => {
      $('#createItemModal').modal('toggle');
      this._api.get('/api/loaisanpham/get-by-id/' + row.maloai).takeUntil(this.unsubscribe).subscribe((res: any) => {
        this.item = res;
        this.formdata = this.fb.group({
          'Maloai':[row.maloai],
